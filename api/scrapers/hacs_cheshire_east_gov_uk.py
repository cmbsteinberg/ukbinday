...
class Source:
    def __init__(self, uprn=None, postcode=None, house_number=None, verify=True):
        self._uprn = uprn
    ...
    async def fetch(self):
        ...
        if self._postcode and self._house_number:
            # Lookup postcode and number to get UPRN
            params = {
                "postcode": self._postcode,
                "propertyname": self._house_number,
            }
            r = await session.get(
            ...
            if s is None or s["data-uprn"] is None:
                raise SourceArgumentExceptionMultiple(
                    ["postcode", "house_number"], "address not found"
                )
        ...
        if self._uprn is None:
            raise SourceArgumentException(
                "uprn",
                "uprn not set but required if postcode and house_number are not set",
            )
