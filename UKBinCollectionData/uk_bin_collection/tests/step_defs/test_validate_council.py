...
from uk_bin_collection.uk_bin_collection.common import RequestTracker
...
def scrape_step(
    ...
    # Track all requests made during scraping
    tracker = RequestTracker(
        council_name=context.council, output_dir="uk_bin_collection/tests/request_logs"
    )

    # Add metadata for reference
    tracker.add_metadata(**context.metadata)
    ...
    with tracker:
        try:
            ...
            log_file = tracker.save_to_file(success=success)
            logging.info(
                f"  └─ Saved {len(tracker.get_all_requests())} requests to: {log_file}"
            )

    context.requests = tracker.get_all_requests()
