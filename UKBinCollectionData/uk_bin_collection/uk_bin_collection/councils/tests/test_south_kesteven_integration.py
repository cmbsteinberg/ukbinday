...
class TestSouthKestevenIntegration:
    ...
    def test_real_postcode_lookup(self):
        ...
        try:
            result = self.council.parse_data("", postcode=self.test_postcode)
    ...
    def test_invalid_postcode_handling(self):
        ...
        try:
            with pytest.raises(ValueError, match="Could not determine collection day"):
                self.council.parse_data("", postcode="INVALID_POSTCODE")
        except Exception as e:
            pytest.skip(f"Integration test failed (likely due to network issues): {e}")
    ...
    def test_collection_day_extraction(self):
        ...
        try:
            collection_day = self.council.get_collection_day_from_postcode(
                None, self.test_postcode
            )
            assert collection_day is not None
            assert collection_day in [
                "Monday",
                "Tuesday",
                "Wednesday",
                "Thursday",
    ...
    def test_green_bin_info_extraction(self):
        ...
        try:
            green_bin_info = self.council.get_green_bin_info_from_postcode(
                None, self.test_postcode
            )

            if (
                green_bin_info
            ):  # Green bin service might not be available for all postcodes
                assert "day" in green_bin_info
                assert "week" in green_bin_info
                assert green_bin_info["day"] in [
                    "Monday",
                    "Tuesday",
                    "Wednesday",
                    "Thursday",
    ...
    def test_collection_date_calculation_accuracy(self):
        ...
        with patch(
            ...
            expected_monday_1 = (test_date + timedelta(days=5)).strftime(
                "%d/%m/%Y"
            )  # Jan 15
            expected_monday_2 = (test_date + timedelta(days=12)).strftime(
                "%d/%m/%Y"
            )  # Jan 22
            ...
            expected_friday_1 = (test_date + timedelta(days=2)).strftime(
                "%d/%m/%Y"
            )  # Jan 12
            expected_friday_2 = (test_date + timedelta(days=9)).strftime(
                "%d/%m/%Y"
            )  # Jan 19
