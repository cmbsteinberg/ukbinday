...
class TestSouthKestevenDistrictCouncil:
    ...
    def test_get_next_collection_dates_monday(self):
        ...
        with patch(
            "uk_bin_collection.uk_bin_collection.councils.SouthKestevenDistrictCouncil.datetime"
        ) as mock_datetime:
    ...
    def test_get_collection_day_from_postcode_success(self):
        ...
        with patch.object(
            self.council, "_get_collection_day_requests"
        ) as mock_requests:
    ...
    def test_get_green_bin_info_from_postcode_success(self):
        ...
        # Mock the requests-based approach
        with patch.object(
            self.council, "_get_green_bin_info_requests"
        ) as mock_requests:
    ...
    def test_parse_data_success_with_green_bin(self):
        ...
        with patch.object(
            self.council, "get_collection_day_from_postcode"
        ) as mock_get_day:
            with patch.object(
                self.council, "get_green_bin_info_from_postcode"
            ) as mock_get_green:
                with patch.object(
                    self.council, "get_next_collection_dates"
                ) as mock_get_dates:
                    with patch.object(
                        self.council, "get_green_bin_collection_dates"
                    ) as mock_get_green_dates:
                        with patch.object(
                            self.council, "parse_calendar_images"
                        ) as mock_calendar:
                            with patch.object(
                                self.council, "get_bin_type_from_calendar"
                            ) as mock_bin_type:
                                mock_get_day.return_value = "Monday"
                                mock_get_green.return_value = {
                                    "day": "Tuesday",
                                    "week": 2,
                                }
                                ...
                                mock_get_green_dates.return_value = [
                                    "09/01/2025",
                                    "13/02/2025",
                                ]
                                mock_calendar.return_value = {
                                    "2025": {"1": {"1": "Black bin", "2": "Silver bin"}}
                                }
                                ...
                                expected = {
                                    "bins": [
                                        ...
                                        {
                                            "type": "Green bin (Garden waste)",
                                            "collectionDate": "09/01/2025",
                                        },
                                        {
                                            "type": "Green bin (Garden waste)",
                                            "collectionDate": "13/02/2025",
                                        },
                                    ]
                                }
                                assert result == expected
                        ...
                        with patch.object(
                            ...
                            expected = {
                                "bins": [
                                    {
                                        "type": "Black bin (General waste)",
                                        "collectionDate": "12/01/2025",
                                    },
                                    {
                                        "type": "Black bin (General waste)",
                                        "collectionDate": "19/01/2025",
                                    },
                                ]
                            }
                            assert result == expected
    ...
    def test_parse_data_collection_day_failure(self):
        ...
        with patch.object(
            ...
            with pytest.raises(
                ValueError,
                match="Could not determine collection day for postcode INVALID",
            ):
    ...
    def test_week_of_month_calculation(self):
        ...
        for date, expected_week in test_cases:
            ...
            assert week_of_month == expected_week, (
                f"Date {date} should be week {expected_week}, got {week_of_month}"
            )
    ...
    def test_days_of_week_mapping(self):
        ...
        for day_name, expected_weekday in days_of_week.items():
            ...
            assert test_date_for_day.weekday() == expected_weekday, (
                f"{day_name} should map to weekday {expected_weekday}"
            )
