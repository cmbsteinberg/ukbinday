...
class CouncilClass(AbstractGetBinDataClass):
            ...
            for row in rows:
                ...
                if cells:
                    ...
                    bin_collection_data.append(
                        {"collectionDate": formatted_date, "type": bin_type}
                    )
    ...
    def smart_select_address(self, driver, house_number: str):
        ...
        for option in dropdown.options:
            ...
            if option_text_clean == user_input_clean or option_text_clean.startswith(
                f"{user_input_clean},"
            ):
                try:
                    option.click()
                    ...
                    for fresh_option in dropdown.options:
                        if fresh_option.text.lower().strip() == option_text_clean:
                            ...
                            found = True
                            print(
                                f"Strict match clicked after refresh: {fresh_option.text.strip()}"
                            )
                            break
                    ...
                    try:
                        ...
                        for fresh_option in dropdown.options:
                            if fresh_option.text.lower().strip() == option_text_clean:
                                ...
                                found = True
                                print(
                                    f"Fuzzy match clicked after refresh: {fresh_option.text.strip()}"
                                )
                                break
