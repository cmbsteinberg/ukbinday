    ...
    def parse_data(self, page: str, **kwargs) -> dict:
        ...
        try:
            ...
            if (
                ...
                or "Verify you are human" in driver.page_source
            ):
                ...
                inputElement_postcode = WebDriverWait(driver, 30).until(
                    EC.presence_of_element_located(
                        (
                            By.ID,
                            "RENFREWSHIREBINCOLLECTIONS_PAGE1_ADDRESSLOOKUPPOSTCODE",
                        )
                    )
                )
                inputElement_postcode.send_keys(user_postcode)
                ...
                if (
                    ...
                    while time.time() - start_time < max_wait:
                        ...
                        if (
                            "Just a moment" not in current_source
                            and "Verify you are human" not in current_source
                        ):
                        ...
                        try:
                            turnstile_elements = driver.find_elements(
                                By.CSS_SELECTOR,
                                "iframe[src*='turnstile'], div[id*='turnstile'], input[name*='turnstile']",
                            )
                        ...
                        print(
                            "Cloudflare challenge timeout - attempting to continue anyway"
                        )
            ...
            try:
                ...
                print(
                    "Collection details not found, checking for any collection content"
                )
                ...
                try:
                    WebDriverWait(driver, 20).until(
                        EC.presence_of_element_located(
                            (
                                By.XPATH,
                                "//*[contains(text(), 'collection') or contains(text(), 'Collection') or contains(text(), 'bin') or contains(text(), 'Bin')]",
                            )
                        )
                    )
            ...
            if not next_collection_div:
                ...
                if (
                    ...
                    print(
                        "WARNING: Still on Cloudflare challenge page - this council may need manual intervention"
                    )
                    ...
                    if collection_text:
                        print(
                            "Found collection-related text but not in expected format"
                        )
                        data["bins"].append(
                            {
                        ...
                        raise ValueError(
                            "Could not find next collection div - saved debug_renfrewshire.html"
                        )

            next_collection_date_elem = next_collection_div.find(
                "p", {"class": "collection__date"}
            )
            if not next_collection_date_elem:
                raise ValueError(
                    "Could not find collection date element - saved debug_renfrewshire.html"
                )
