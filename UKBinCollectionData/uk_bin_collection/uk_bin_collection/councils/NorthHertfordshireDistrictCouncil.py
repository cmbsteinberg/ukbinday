...
MOBILE_API_BASE = (
    "https://apps.cloud9technologies.com/northherts/citizenmobile/mobileapi"
)
...
def lookup_uprn(postcode: str, paon: str) -> str:
    ...
    try:
        response = requests.get(
            url, headers=MOBILE_API_HEADERS, timeout=30, params={"postcode": postcode}
        )
...
class CouncilClass(AbstractGetBinDataClass):
    ...
    def parse_data(self, page: str, **kwargs) -> dict:
        ...
        for container_num in range(1, MOBILE_API_NUM_CONTAINERS + 1):
            ...
            bin_type = container.get(
                "containerDescription", f"Container {container_num}"
            )
            try:
