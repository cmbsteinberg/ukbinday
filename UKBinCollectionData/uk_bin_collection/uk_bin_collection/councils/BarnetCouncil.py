    ...
    def parse_data(self, page: str, **kwargs) -> dict:
        ...
        try:
            ...
            try:
                WebDriverWait(driver, 15).until(
                    EC.presence_of_element_located(
                        (
                            By.XPATH,
                            "//div[contains(text(), 'Next collection') or contains(text(), 'collection date')]",
                        )
                    )
                )
            except:
            ...
            collection_divs = soup.find_all(
                "div", string=re.compile(r"Next collection date:")
            )
            ...
            for collection_div in collection_divs:
                try:
                    ...
                    parent_div = (
                        collection_div.parent
                        if collection_div.parent
                        else collection_div
                    )
                    ...
                    for i, line in enumerate(lines):
                        ...
                        if "Next collection date:" in line:
                            ...
                            date_match = re.search(
                                r"Next collection date:\s+(.*)", line
                            )
                            if date_match:
                                collection_date_string = (
                                    date_match.group(1).strip().replace(",", "")
                                )
                            break

                    if collection_date_string:
                        if collection_date_string in overrides_dict:
                            collection_date_string = overrides_dict[
                                collection_date_string
                            ]

                        current_date = datetime.now()
                        parsed_date = datetime.strptime(
                            collection_date_string + f" {current_date.year}",
                            "%A %d %B %Y",
                        )
