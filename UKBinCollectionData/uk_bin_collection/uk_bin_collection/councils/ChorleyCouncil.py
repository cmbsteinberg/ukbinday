        ...
        try:
            ...
            check_button = WebDriverWait(driver, 10).until(
                EC.element_to_be_clickable(
                    (
                        By.XPATH,
                        "//a[@class='button' and @href='https://forms.chorleysouthribble.gov.uk/chorley-bincollectiondays']",
                    )
                )
            )
            check_button.click()
            ...
            lookup_button = WebDriverWait(driver, 10).until(
                EC.element_to_be_clickable(
                    (By.XPATH, "//button[contains(@class, 'btn--lookup')]")
                )
            )
            lookup_button.click()
            ...
            next_button = WebDriverWait(driver, 10).until(
                EC.element_to_be_clickable(
                    (By.CSS_SELECTOR, "button[type='submit'][value='Next']")
                )
            )
            next_button.click()
                ...
                for i, row in enumerate(rows):
                    ...
                    if i > 0 and len(cells) >= 2:  # Skip header row
                        ...
                        if (
                            collection_type
                            and collection_date
                            and collection_date != "Collection"
                        ):
