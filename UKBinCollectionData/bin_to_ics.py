...
def parse_time_delta(time_str: str) -> datetime.timedelta:
    ...
    # Handle hours
        ...
        time_str.endswith("h")
        or time_str.endswith("hour")
        or time_str.endswith("hours")
    ):
    ...
    # Handle minutes
        ...
        time_str.endswith("m")
        or time_str.endswith("min")
        or time_str.endswith("mins")
        or time_str.endswith("minutes")
    ):
    ...
    # Default to hours if no unit specified
        ...
        try:
            ...
            raise ValueError(
                f"Invalid time format: {time_str}. Use format like '1d', '2h', or '30m'."
            )
    ...
    for collection_date, bin_types in collections_by_date.items():
        ...
        if all_day:
            ...
            collection_datetime = datetime.datetime.combine(
                collection_date, datetime.time(7, 0, 0)
            )
...
def main():
    parser = argparse.ArgumentParser(
        description="Convert UK Bin Collection Data to ICS calendar file."
    )
    parser.add_argument(
        "--input", "-i", help="Input JSON file (if not provided, read from stdin)"
    )
    parser.add_argument(
        "--output", "-o", help="Output ICS file (default: bin.ics)", default="bin.ics"
    )
    parser.add_argument(
        "--name",
        "-n",
        help="Calendar name (default: Bin Collections)",
        default="Bin Collections",
    )
    parser.add_argument(
        "--alarms",
        "-a",
        help='Comma-separated list of alarm times before event (e.g., "1d,2h,30m")',
    )
    parser.add_argument(
        "--no-all-day",
        action="store_true",
        help="Create timed events instead of all-day events",
    )
    ...
    calendar = create_bin_calendar(
        bin_data, args.name, alarm_times=alarm_times, all_day=not args.no_all_day
    )
